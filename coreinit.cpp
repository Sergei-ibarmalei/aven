...
void Game::startInit()
{
	...
	fleet_AlienOne = new Fleet_AlienOne{ mt,
										...
										ALIENONEFLEET_SIZE };
	if (fleet_AlienOne->Init_ok() == false) { init_ok = false; return; }
	sky = new Sky{};
	mainmenu =
		new MainMenu{ textureStore->operator[](enumTextureType::mainmenuText) };
	if (mainmenu->Init_ok() == false) { init_ok = false; return; }
	...
	hero = new Hero{ textureStore->operator[](enumTextureType::ships),
		heroIntroStartPoint };
	if (hero->Init_ok() == false) { init_ok = false; return; }
